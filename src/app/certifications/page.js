...
export default function CertificationsPage() {
  return (
    <div style={{ padding: '110px 24px 90px', maxWidth: '1100px', margin: '0 auto' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(320px, 1fr))', gap: '20px' }}>
        {certifications.map(({ title, issuer, date, fileUrl, type, color }) => (
          <div key={title} className="card-glass"
            style={{ borderRadius: '18px', overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>

            {/* Accent bar */}
            <div style={{ height: '3px', background: `linear-gradient(to right, ${color}, ${color}33)` }} />
            ...
            <div style={{ padding: '24px', flex: 1, display: 'flex', flexDirection: 'column' }}>
              ...
              <h2 style={{ fontSize: '0.97rem', fontWeight: 700, color: '#f1f5f9', marginBottom: '6px', lineHeight: 1.4 }}>{title}</h2>
              <p style={{ fontSize: '0.78rem', fontWeight: 700, color, marginBottom: '18px', letterSpacing: '0.03em' }}>{issuer}</p>
